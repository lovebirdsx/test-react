...
function jsonPathToString(jsonPath: string[]): string {
    return jsonPath.join('.');
}

function stringToJsonPath(str: string): string[] {
    if (!str) {
        return [];
    }
    return str.split('.');
...
function setValueByJsonPath(obj: any, jsonPath: string[], value: any): void {
...
function addFieldToJsonPathString(jsonPath: string, field: string): string {
...
const createObjectSlice: StateCreator<ObjectSlice> = (set) => {
    const data = {
        foo: {
            id: '123',
            name: 'baz',
...
const useEditorStore = create<ObjectSlice>((set, get, api) => ({
